...
if ($result1->num_rows > 0) {
   ...
   while ($row = mysqli_fetch_assoc($result1)) {
    ...
    $serviceId = $row['id'];
    $invoiceQuery = "SELECT * FROM invoice WHERE service_id = ?";
    ...
    $invoiceQuery = "SELECT `invoice_id`, SUM(`paid_amount`) AS total_paid, `total_amount` FROM `invoice` WHERE `invoice_id` = ? GROUP BY `invoice_id`";
$stmt = $conn->prepare($invoiceQuery);
$stmt->bind_param("s", $invoiceId);  // Bind the invoice_id
$stmt->execute();
$invoiceResult = $stmt->get_result();
...
// If invoice exists, calculate total paid, total amount, and due amount
if ($invoiceRow) {
    $totalPaidAmount = $invoiceRow['total_paid'] ?? 0; // Default to 0 if null
    $totalAmount = $invoiceRow['total_amount'] ?? 0; // Default to 0 if null
...
$fromDate = DateTime::createFromFormat('Y-m-d', $row['from_date'])->format('d/m/Y');
$endDate = DateTime::createFromFormat('Y-m-d', $row['end_date'])->format('d/m/Y');
        echo "<tr class='dataTable_row'>
                ...
                <td>
                  <strong>Start Date:</strong> {$fromDate}<br>
          <strong>End Date:</strong> {$endDate}
            ...
            <strong>Amount Paid:</strong> " . number_format($totalPaidAmount, 2) . "
        </td>
